...
class MHMMOrchestrator:
    ...
    def __init__(self, track_json_path: str, config_path: str) -> None:
        ...
        self._last_raw_total: float = 0.0
    ...
    def process_fix(
        ...
        _n_gated_this_cycle = 0 # Number of gated measurements this cycle
        for h in self.hypotheses:
            ...
            if not _kf_upd.accepted:
                _n_gated_this_cycle += 1
            # NOTE: No return cause KFState is updated internally.
            # Can be accessed via h.kf.get_state() for likelihood computation and output building.

        self._last_n_gated = _n_gated_this_cycle
        ...
        for h in self.hypotheses:
            # Validate each projection before computing likelihood for each hypothesis.
            if h.latest_projection is None:
                logger.warning(
    ...
    def _update_weights(self) -> None:
        ...
        self._last_raw_total = total  # exposed for diagnostics publish in OLE
